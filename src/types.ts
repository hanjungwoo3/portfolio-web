...
export interface Price {
  ...
  priceNative?: number;     // 현지통화 보조표기(현재가) — 미국 외 해외 구성종목(원화 환산분의 현지값)
  nativeCurrency?: string;  // 현지통화 코드 "JPY" | "EUR" | "GBp"(펜스!) | "SEK" …
  fxSymbol?: string;        // 찾아낸 상장 심볼(6857.T / BA.L) — 이름 매칭이 틀렸는지 카드에서 눈으로 확인
