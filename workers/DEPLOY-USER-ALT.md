...
| | **Deno Deploy** | Cloudflare | Netlify | Supabase |
|---|---|---|---|---|
| 무료 한도 | 100,000/일 | 100,000/일 | Edge Functions 무료 한도 | 500,000/월 |
| 배포 방법 | **브라우저만** | **브라우저만** | CLI 필요 | CLI 필요 |
| 예상 소요 | **1~2분** | 10분 | 15분 | 15분 |
| 카드 등록 | 불필요 | 불필요 | 불필요 | 불필요 |

**가장 쉬운 건 [Deno Deploy](deno-proxy/README.md) 입니다.** GitHub 로 1클릭 가입 →
New Playground → `main.ts` 붙여넣기 → Save & Deploy. 터미널이 필요 없고 1~2분이면 끝납니다.
Cloudflare 도 브라우저만으로 되지만 가입 절차(이메일 인증·계정 타입 선택)가 길어 10분쯤 걸립니다.

egress IP 풀도 Cloudflare 와 다르기 때문에, 위에서 말한 "다른 공급자를 하나 두는" 조건도
Deno 하나로 만족합니다. Cloudflare 워커를 하나 더 만드는 것보다 낫습니다.
...
### Q6. URL 은 나오는데 앱에 등록하면 "응답 확인 실패한 프록시"

배포가 실패해서 **엣지 함수가 안 올라간** 경우입니다. Netlify 사이트와 URL 은
만들어지므로 주소는 살아 있지만, 프록시 대신 `public/index.html` 정적 페이지가 응답합니다.
앱은 JSON(`{` 또는 `[` 로 시작)을 기대하므로 HTML 을 받으면 실패로 판정합니다.

**확인** — 프록시 URL 을 브라우저 주소창에 그냥 붙여넣어 보세요.

| 보이는 것 | 뜻 |
|---|---|
| `{"ok":true,"message":"포트폴리오 프록시(Netlify)가 정상 작동 중입니다…"}` | 정상. 등록하면 됩니다 |
| `portfolio proxy` 라는 밋밋한 페이지 / 404 | 엣지 함수 미배포 — 배포 로그를 보세요 |

**Windows 에서 배포가 실패했다면** 아래 오류일 가능성이 큽니다.
