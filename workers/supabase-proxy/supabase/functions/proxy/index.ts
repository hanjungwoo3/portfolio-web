// Supabase Edge Function (Deno) — Cloudflare/Vercel/Netlify 워커와 동일 로직의 CORS 프록시
// 사용:  GET|POST {base}/?url=<encoded_target_url>
//   base = https://<project-ref>.supabase.co/functions/v1/proxy
//
// 배포: supabase functions deploy proxy --no-verify-jwt   (공개 프록시 → JWT 검증 끔)
// 무료: 카드 등록 불필요. 무료 한도 50만 호출/월.
...
async function handler(request: Request): Promise<Response> {
  ...
  if (!target) {
    return new Response(JSON.stringify({
      ok: true,
      message: "포트폴리오 프록시(Supabase)가 정상 작동 중입니다. 이 프록시는 앱에서만 시세를 가져올 수 있어요(브라우저 직접 호출은 차단). 앱 ⚙️ 설정 → '내 전용 프록시 URL' 에 이 주소를 등록해 사용하세요.",
    }, null, 2), {
      status: 200,
...
// Supabase Edge 런타임 — Deno.serve 진입점
Deno.serve(handler);
