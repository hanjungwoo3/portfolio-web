...
### worker URL을 브라우저에서 그냥 열어보세요

주소창에 본인 worker 주소를 입력해 열면 다음과 같은 응답이 나오면 **배포 성공**입니다:

```json
{
  "ok": true,
  "message": "포트폴리오 프록시 워커가 정상 작동 중입니다. ..."
}
```

**이 메시지가 보이면 워커가 살아있다는 뜻이에요.** 바로 5단계(앱에 등록)로 넘어가세요.

> ⚠️ 이 worker 는 보안상 **포트폴리오 앱에서 호출할 때만 실제 시세를 가져옵니다.**
>     그래서 주소 뒤에 `?url=...` 를 직접 붙여 브라우저로 열면 `{"error":"Forbidden origin"}` 이 나오는데,
>     이것도 **정상**입니다(앱이 아닌 직접 호출은 차단). 진짜 시세 확인은 5단계대로 앱에서 합니다.

### 검증 표

| 상황 | 응답 | 의미 |
|------|------|------|
| 브라우저로 worker 주소 그냥 열기 | `{"ok":true, "message":"...정상 작동 중..."}` | ✅ 정상 (워커 살아있음) |
| 주소에 `?url=...` 붙여 직접 열기 | `{"error":"Forbidden origin"}` | ✅ 정상 (앱에서만 호출 가능 — 보안) |
| 앱에 등록 후 시세 갱신됨 (5단계) | 시세·차트 정상 표시 | ✅ 완벽 |
| 그냥 열었는데 `Error 1101`·빈 페이지·502 | — | ❌ 배포 실패/코드 오류 — 3.3~3.4 다시 |
...
→ **이건 에러 아니라 정상입니다.** 이 worker 는 보안상 **포트폴리오 앱에서 호출할 때만** 시세를 가져오도록 잠겨 있어요. 주소에 `?url=...` 를 붙여 브라우저로 직접 열면 의도적으로 차단됩니다.

- worker 주소를 **그냥**(파라미터 없이) 열면 → `{"ok":true, "message":"...정상 작동 중..."}` 이 나오는 게 정상 (배포 성공 신호).
- `?url=...` 붙여서 직접 열면 → `Forbidden origin` 이 나오는 게 정상 (앱에서만 호출 가능).

어느 쪽이든 **배포는 성공**한 거예요. 그대로 5단계(앱에 등록)로 진행하고 **앱에서 시세가 갱신되는지**로 확인하세요.
...
시세가 간헐적으로 멈추고 개발자도구에 `400` 이 보인다면, 시세 소스가 **Cloudflare 의 나가는 IP 풀** 을 제한하고 있는 것일 수 있습니다. 이 풀은 전 세계 Cloudflare 고객이 공유해서, 내 호출량이 적어도 같이 막힐 수 있습니다. (실측: 워커 3개가 동시에 400 → 30초 뒤 함께 회복)

이 경우 **워커를 하나 더 만들어도 소용없습니다.** 나가는 IP 가 다른 공급자를 하나 두어야 합니다.

→ **[Netlify / Supabase 프록시 추가 가이드](../DEPLOY-USER-ALT.md)**

여러 개를 등록해두면 앱이 자동으로 분산하고, 한쪽이 막히면 다른 공급자로 넘어갑니다. 다만 두 방법 모두 터미널 사용이 필요하니, 익숙하지 않으시면 안 하셔도 됩니다.
